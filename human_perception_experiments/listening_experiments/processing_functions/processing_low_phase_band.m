function [output_signal, band_limits] = processing_low_phase_band(input_signal, band_limits)
    %% README
    ...
    if nargin == 1
        low = 1;
        high = floor(floor(size(X_phase,1)/2) * 0.333);
    ...
    phase(size(phase,1)-high + 1:end,:) = -flipud(phase(low:high,:) + rand_phase_block);
