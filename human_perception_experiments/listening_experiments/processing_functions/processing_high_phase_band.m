function [output_signal, band_limits] = processing_high_phase_band(input_signal, band_limits)
    %% README
    ...
    if nargin == 1
        low = floor(floor(size(X_phase,1)/2) * 0.666);
        high = floor(floor(size(X_phase,1)/2));
